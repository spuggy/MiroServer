...
public class MiroTeamReport {
    ...
    private int getPiePageCount() {
        ...
        if((size%this.PIESPERPAGE)>0) {
            return (int)Math.floor(size/this.PIESPERPAGE) + 1 ;

        }   else {
            return (int)Math.floor(size/this.PIESPERPAGE)  ;
	...
	public void generateReport(MiroTeam miroTeam) throws Exception {
		...
		this.generateXMLReportFile();
         MiroReportPDFGenerator.generatePDF(this.baseDirectory, this.miroTeam.getMiroTeamNameFileName(""),false, true);
